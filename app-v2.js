...
function renderTodaySchedule() {
  ...
  document.querySelector('#todaySchedule').innerHTML = items.length ? items.map(({ item, air }) => `<button class="today-row" type="button" data-id="${escapeHtml(item.id)}">
    <span class="today-time">${air.time}</span>
    <span class="today-thumb">${posterHtml(item)}</span>
    <span class="today-copy"><strong>${escapeHtml(item.titleThai)}</strong><small class="today-episode">${escapeHtml(latestText(item))}${hasNewEpisode(item) ? ' • มีตอนใหม่' : ''}</small></span>
    <span class="today-channel"><small>ช่องทางรับชม</small><strong>${escapeHtml(channelShort(item.channel))}</strong></span>
    <span class="today-detail">รายละเอียด</span>
