...
- `seasonRank` / `seasonRankPrevious` — written by `update-ranking-snapshot.js` for the current season's Top 10 only (`seasonRankPrevious: 0` means the anime entered the Top 10 since yesterday); both are deleted from anime outside the Top 10, and both are whitelisted in `build-site-data.js` because `app.js` draws the rank-movement arrows from them
